...
;;  | Begin Expr Expr
;;  | Let Id Expr Expr
;;  | LetRec (Binding list) Expr <- This allows us to implement user defined functions in a non-ad-hoc way
;;  | Lam Name [Variable] Expr  <- Name is internal, because we need labels for ASM
;;  | App Expr [Exprs]  <- Changed to Expr because we may call a lambda, which is an expr
;;
